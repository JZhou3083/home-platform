...
from home_platform.rendering import RgbRenderer1
from panda3d.core import LVector3f, TransformState, ClockObject, LVecBase3f, BitMask32, LVector4f,AudioSound,LPoint3f
...
class RoamingRalphDemo(ShowBase):
    def __init__(self):
        ...
        self.inst6 = addInstructions(0.36, "[A]: Rotate face Left")
        self.inst7 = addInstructions(0.42, "[D]: Rotate face Right")
...
#        self.environ.reparentTo(render)
        ...
        self.scene = SunCgSceneLoader.loadHouseFromJson(houseId="0004d52d1aeeb8ae6de39d6bd993e992", datasetRoot=SUNCG_DATA_DIR)
        self.environ = self.scene.scene
        self.environ.reparentTo(render)
        cameraTransform = TransformState.makePosHpr(LVector3f(0.0,  -0.3, 0),LVector3f(0, 180, 0))
        self.renderer = RgbRenderer1(self.scene, size=(128, 128), fov=70.0, cameraTransform=None)
    	self.renderer.showRoomLayout(showCeilings=False, showWalls=True, showFloors=True)#        self.scene=SunCgSceneLoader.loadHouseFromJson(houseId="0004d52d1aeeb8ae6de39d6bd993e992", datasetRoot=SUNCG_DATA_DIR)
...
#        ralphStartPos = self.environ.find("**/start_point").getPos()
        ralphStartPos = LPoint3f((42.5, -39.1, 0))
#        self.ralph = Actor("models/ralph",
#                           {"run": "models/ralph-run",
#                            "walk": "models/ralph-walk"})
        self.ralph = Actor("models/eve",
                           {"run": "models/eve_run",
        ...
        self.camera.setPos(self.ralph.getX(), self.ralph.getY() - 10, -0.4)
	...
	if self.keyMap["head-left"]:
	    self.Neck.setP(self.Neck,np.pi/4)
	    self.camera.setH(self.camera, np.pi/4)
